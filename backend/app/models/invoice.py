...
class Invoice(Base):
    ...
    # 关联差旅细项：差旅类发票挂到差旅申请单的某一条费用细项上（镜像采购细项）
    travel_requisition_item_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("travel_requisition_items.id"), nullable=True, index=True
    )
