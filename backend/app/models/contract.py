...
class HRContract(Base):
    ...
    id_number: Mapped[str | None] = mapped_column(String(30))  # 身份证，脱敏 * 原样保留
    contract_type: Mapped[str] = mapped_column(String(30), default="劳动合同")
    party_a: Mapped[str | None] = mapped_column(String(200))  # 公司
    party_b: Mapped[str | None] = mapped_column(String(100))  # 员工
    start_date: Mapped[date | None] = mapped_column(Date)
    end_date: Mapped[date | None] = mapped_column(Date)
    status: Mapped[str] = mapped_column(String(20), default="生效")  # 生效/到期/终止/续签中
    salary: Mapped[Decimal | None] = mapped_column(Numeric(18, 2))
...
class PurchaseContract(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    contract_no: Mapped[str | None] = mapped_column(String(50))
    supplier_id: Mapped[int | None] = mapped_column(ForeignKey("parties.id"))
    sign_date: Mapped[date | None] = mapped_column(Date)
    effective_date: Mapped[date | None] = mapped_column(Date)
    expire_date: Mapped[date | None] = mapped_column(Date)
    amount: Mapped[Decimal | None] = mapped_column(Numeric(18, 2))
