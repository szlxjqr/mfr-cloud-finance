...
- **最后更新**：2026-07-21
- **Git HEAD**：`a41588f`（main；尚未 push 到 GitHub，因当前沙箱无法连接 github.com:443）
- **更新内容**：
  1. 新增发票报销模块 P0+P1+P2 闭环（进项发票后端持久化、InvoiceInput.vue 接真实后端、发票↔报销单关联、归档上传、凭证草稿）。
  2. 更新 §3 进度：完整页 17→19，占位页 17→15；新增「发票报销模块进度」表。
  3. §10 存储位置规范不变；本文件仍为 GitHub 权威副本，但**最新 commit 尚未同步到远程**，需用 §11.2 的补丁或 T 网盘方式同步。
- **对应提交**：`a41588f`（发票报销功能）
- **同步补丁**：`/Users/szlxjqr/Workbuddy/Claw/invoice-reimburse-feature.patch`（`git am < patch` 即可应用）
