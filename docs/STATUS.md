...
- **最后更新**：2026-07-21
- **Git HEAD**：`1ae8ca8`（main；本地已 commit，因 GitHub 443 暂时不可达尚未 push，待网络恢复后补推）
...
- **坑**：① `npm run build` 的 `vue-tsc -b` 类型校验——`ReimbursementBill.bill_no` 为 `string | null | undefined`，赋给 `form.bill_no`（`string | null`）需 `?? null` 兜底；② 8521 端口若被上次会话遗留的旧后端占用，新进程会 bind 失败（`address already in use`），需先 `lsof -ti tcp:8521 | xargs kill` 再起；③ `git push` 仍走 `GIT_HTTP_VERSION=1.1`（见 §4，HTTPS 远程）；④ 当前环境 GitHub 443 不可达（`Couldn't connect to server`），本次改动已本地 commit，待网络恢复后补推。
