...
- **最后更新**：2026-07-21
- **Git HEAD**：`0bdb981`（main；已 push 到 GitHub 远程）
...
- **更新内容**：
  ...
  21. 老板反馈打印超出 A4 一页导致首屏顶部被裁切。根因：`@media print` 未声明 `@page` 纸张尺寸，浏览器按系统默认纸张（如 Letter）缩放/分页，内容溢出裁掉顶部。UI Designer 重做打印分页：① 显式 `@page { size: A4; margin: 14mm 16mm }` 与屏幕内边距一致；② `.expense-form` 打印态 `width:auto`（不再固定 210mm 致横向溢出）、`min-height:0`（自然流式分页，内容多长印几页）；③ `print-color-adjust: exact` 保留浅灰底（类型带/存根区）灰度呈现；④ 区块/标题 `break-after:avoid` 防孤行；⑤ 发票卡片 `break-inside:avoid` 三写兜底（page-break-inside / webkit-column-break-inside），放不下整卡移至下一页、宁留白也不拆卡。构建通过（改动 BillDetail.vue 打印样式）。
- **对应提交**：`a41588f`（发票报销功能）、`aaaae4c`（STATUS 进度更新）、`21f909e`（单号+增加发票）、`e0fd4b8`（STATUS）、`95dd9bc`（重复校验修复）、`df3ce53`（16位发票编码+并发安全生成器）、`3ab900d`（一级审批流程）、`475fda7`（我的报销+物品报销单详情）、`5ad4a7a`（STATUS 记录）、`f4faf53`（详情页版式微调）、`3a5f301`（一行一发票汇总式）、`c55a157`（发票方框卡片）、`bc3ad58`（火车票风卡片）、`a8b461b`（卡片布局优化：编码日期露全/类型缩写/价税区缩小/物品字号调小）、`de17513`（发票号码+开票日期分行）、`83c8d6c`（类型标签改为号码后缀）、`5de3faa`（类型徽章移到左侧空白区并放大美化）、`206057b`（登机牌式紧凑发票卡片）、`2fd7fc8`（黑白灰度打印友好方案，物品改为内容）、`81cd147`（发票卡片定稿打磨）、`019b3ae`（采购报销单打印版式整体重排）、`b811137`（汇总表头换行：报销金额/支付金额（元）分两行）、`1ae8ca8`（修复发票识别购销方互换：按标签定位提取 buyer/seller）、`11d9acd`（chore：开发库 smart_finance.db 纳入 git，.gitignore 加例外）、`0bdb981`（fix：A4 打印分页——@page A4 + 发票卡片整卡不跨页 + 标题表格防孤行）
