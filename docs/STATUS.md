...
- **最后更新**：2026-07-21
- **Git HEAD**：`aaaae4c`（main；**已 push 到 GitHub 远程**，区间 a41588f..aaaae4c）
- **更新内容**：
...
- **对应提交**：`a41588f`（发票报销功能）、`aaaae4c`（STATUS 进度更新）
- **坑**：首次 `git push` 走默认 HTTP/2 多路复用时卡住超时（"Failed to connect ... 443"），curl 直连 443 反而正常 → 加 `GIT_HTTP_VERSION=1.1` 立刻成功（已记入 §4）
