...
- **最后更新**：2026-07-19
- **Git HEAD**：`10017fd`（main；其下 `2821a26`+.gitignore 例外、`b1ba921` 引入完整前端依赖入 git）
- **更新内容**：完整前端依赖 `frontend/node_modules` 已提交进 git（约 1.4 万文件，靠 .gitignore 的 `!frontend/node_modules/` 例外放行）；移除仓库根残缺的 `node_modules/` 与桩 `package.json`；同步修正本文件 §2/§8（新任务 clone 通常无需 `npm install` 即可直接 build）
- **对应提交**：`2821a26`（.gitignore 例外）、`b1ba921`（依赖入 git）、`10017fd`（本文件同步）
