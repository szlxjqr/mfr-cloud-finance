...
| **Git 仓库根** | `/workspace`（项目在子目录 `mfr-cloud-finance/`；仓库根另含一份已提交但**残缺**的 `node_modules/`，构建用不到） |
...
> **本地环境一致性（重要，别担心要"学配置"）**：新任务的本地环境和本任务**开箱即用、基本一致**——Node/npm/Python 运行时、全局工具（`http-server`/`typescript`/`pnpm` 等，均为 CloudStudio 镜像预装）、所有源码与构建配置（在 git 里）都一致，无需从我这里学任何本地配置窍门。需要说明的本地状态：(a) 依赖：`mfr-cloud-finance/frontend/node_modules`（真正的前端依赖，含 vite/vue-tsc/typescript）**正确被 gitignore，未进 git**；仓库根另有一份已提交的 `node_modules/`（约 250M，64 个包）但**残缺（缺 vite/typescript 等），不能用于构建**——所以 clone 后**仍需在 frontend 跑 `npm install`**（步骤 5）；(b) SSH：`~/.ssh/config`（443 路由）与私钥——仅用于**推送**，本文件已写死做法，照抄即可。
> 最小起步（只看不改、不推送时）：`git clone`(HTTPS 公开库，无需 SSH) → `cd mfr-cloud-finance/frontend && npm install` → `npm run build` → 起预览。SSH/config/key 只在你想 push 时才需要。
...
5. **装依赖 + 构建**（clone 后必做；仓库根的 `node_modules` 残缺不可用，必须本地重装）：
   ```bash
   cd /workspace/mfr-cloud-finance/frontend
   npm install          # 需要能访问 npm registry（registry.npmjs.org）；会生成 frontend/node_modules
