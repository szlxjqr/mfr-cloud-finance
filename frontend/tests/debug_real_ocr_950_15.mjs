...
const buildDir = path.join(root, '.invoice-test-build-real950-15')
...
const { extractInvoiceFieldsV2, dualRecognize, attachValidation } = await import(pathToFileURL(path.join(buildDir, 'invoiceDual.js')).href)

const text = readFileSync(path.join(__dirname, 'fixtures/invoices/flight_itinerary_950_ocr_1_5x.txt'), 'utf8')
const gate = dualRecognize(text, extractInvoiceFields, extractInvoiceFieldsV2)
console.log('=== 950 1.5x OCR dualRecognize ===')
...
const v1 = attachValidation(r1)
console.log('attachValidation(r1):', JSON.stringify(v1, null, 2))
