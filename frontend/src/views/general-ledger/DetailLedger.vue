...
import { listSubjects, getSubsidiaryLedger } from '@/api/ledger'
import type { AccountSubject, SubsidiaryLedgerOut } from '@/types/ledger'
...
const displayData = computed(() => {
  if (!hideZeroOpening.value) return tableData.value
  return tableData.value.filter(r => !(r.date === '' && r.balance === 0))
})
...
/** 加载选中科目的明细账（真实数据，逐笔分录） */
async function loadSubsidiary(account: AccountNode) {
  loading.value = true
  try {
    const res = await getSubsidiaryLedger(account.code, period.value || undefined)
    const out: SubsidiaryLedgerOut = res.data
    const entries: DetailEntry[] = []
    // 期初余额行（非零才显示）
    if (out.opening_debit !== 0 || out.opening_credit !== 0) {
      entries.push({
        id: 'e0', date: '', voucherWord: '', voucherNumber: 0,
        summary: '期初余额',
        debitAmount: out.opening_debit,
        creditAmount: out.opening_credit,
        direction: out.opening_debit > 0 ? '借' : '贷',
        balance: Math.max(out.opening_debit, out.opening_credit),
      })
    }
    out.lines.forEach((ln, i) => {
      const parts = ln.voucher_no.split('-')
      const word = parts[0]
...
onMounted(async () => {
  ...
  const code = (route.query.accountCode as string) || ''
  if (code) {
    const hit = flatAccountList.value.find(a => a.code === code)
    if (hit) selectAccount(hit)
  }
})
</script>
