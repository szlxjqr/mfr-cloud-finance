<template>
  <div class="travel-form" ref="formRef">
    ...
    <table class="info-table base-table">
      ...
      <tr>
        <td class="label">归属研发</td><td colspan="5">{{ p.is_rd_project || '否' }}<template v-if="p.is_rd_project === '是' && p.rd_project_code">（{{ p.rd_project_code }}）</template></td>
      </tr>
...
const props = defineProps<{ row: TravelReq }>()
const p = computed(() => props.row)
